...
public class MainMenu_Mon extends aConsoleView {

	public MainMenu_Mon(ConsoleUI manager) {
		super(manager);
		Log("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
	}
	...
	public void init() {
		...
		this.options.put("3", "CREATURE_DEX");
	}
	...
	public boolean handle(String inp) {
		...
		if (inp.equals("3")) {
			this.manager.Session.setView(new CreatureDexView(this.manager));
			return true;
		}
