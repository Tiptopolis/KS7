package prime._METATRON.Resources;
...
import prime.Core.DefaultResources;
import prime.Core.GdxFileUtils;
import prime._METATRON.Core.GlyphSheet;
...
public class FontAtlas {
	...
	public static BitmapFont getFont(String name, Integer size) {
		...
		for (Entry<BitmapFont, Integer> e : FontCache.entrySet()) {
			BitmapFont f = e.getKey();
			int s = e.getValue();

			if ((f.getData().name.equals(name+"-"+size) || f.getData().name == name+"-"+size) && s == size)
				return f;
		}
