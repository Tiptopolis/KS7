...
setMoCam();
	...
	int n = (i+1)%posAr.length();
	 pct = (distance((posAr[i].xy/u_resolution),mouseAt)-0.1);
		if(pct >= 0.1)
		{
		vec3 c = vec3(pct);
		color = vec4(c,1);
		...
		{
		 pct = (distance(posAr[i].xy/u_resolution,posAr[n].xy/u_resolution)-0.1);
		vec3 c = vec3(pct);
		color = vec4(c,1);
		}
	}
	...
	//gl_FragColor = vec4(vec3(fract(pct*10.0)),1.0);
